...
### 0. Подготовка
⚠️ **Важно:** Убедитесь, что файл `internal-0.0.1-SNAPSHOT.jar` находится в папке `target/`

Если файл отсутствует, поместите его туда перед запуском тестов.

### 1. Запустить приложение
```bash
java -jar -Dsecret=qazWSXedc -Dmock=http://localhost:8888/ target/internal-0.0.1-SNAPSHOT.jar
```
